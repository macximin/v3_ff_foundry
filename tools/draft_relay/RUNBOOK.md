...
이 smoke는 원고 발주나 run-dir 생성을 하지 않고 B-Rail의 엔딩 경로, 현재 B의 5화 상한, status/B-Rail/Corridor pointer, 종결 뒤 내구 필드 재검증·변동 필드 무효화 계약을 검사한다.
...
현재 화 입력은 정확한 `03_episode_bet/epNNN_episode_bet.md`이고 `상태: committed`여야 한다. `00_status.md`의 `current_b_arc`는 B-Rail의 active B와 Rolling Corridor의 current B에 모두 일치해야 한다. Relay는 B의 시작·예상 종결·하드 종결, `arc_episode_cap: 5`, next B의 provisional 상태, 엔딩까지의 B-Rail, owner 승인 종결 뒤 내구 필드 재검증·변동 필드 무효화 계약을 검사한다. 요청 화가 active B의 최대 5화 범위를 벗어나면 발주 전에 hard-fail한다. 직전 endpoint는 `04_manuscript/manifest.yaml`의 정확한 episode entry 하나가 지정한 `repo_snapshot`과 SHA-256만 확인한다. imported bridge에서 그 entry가 legacy snapshot을 가리키는 것은 작품 adapter의 책임이다. 전체 manifest와 Narrative State 정합성은 relay가 아니라 doctor가 검증한다.
