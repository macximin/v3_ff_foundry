...
B-Rail은 현재 예상 엔딩까지 이어지는 Story Arc의 순서표다. 각 B는 승인 원고 기준 1~5화 안에 중심 질문을 결산한다. 먼 B는 확정 줄거리가 아니라 장기 경로 가설이다.
...
- 엔딩 상단이 250화이고 첫 B가 3화로 닫혔다면 최대 5화 상한상 최소 51개 B 슬롯의 용량이 필요하다. 이는 화수 확정이 아니라 누락 없는 경로 용량 검사다.
